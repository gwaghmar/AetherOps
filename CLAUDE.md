...
## Workflow rule (read before editing)

`AGENTS.md` mandates that edits be routed through a GSD slash command — `/gsd-quick` for small fixes/docs, `/gsd-debug` for investigation, `/gsd-execute-phase` for planned phase work. Do not invoke Edit/Write outside one of these unless the user explicitly says to bypass GSD.
...
| `src/proxy.ts` | Refreshes Supabase session on every request (must not contain custom logic between `createServerClient` and `getUser`). Next.js 16 replaced `middleware.ts` with `proxy.ts` — do not re-create the old file. |
...
| Email | Resend |
| Errors | Sentry (`sentry.{client,server,edge}.config.ts`) — `beforeSend` strips `user.email` and `user.ip_address` before send; gated on `NEXT_PUBLIC_SENTRY_DSN` |
| Local Supabase | `supabase/config.toml` + `docker-compose.yml` available; dev typically points at hosted Supabase via `.env.local` rather than local stack |
...
All workers live under `src/app/api/internal/worker/` and verify `CRON_SECRET`.

| Path | Schedule | Purpose |
|---|---|---|
| `/api/internal/worker/fulfillment` | `0 0 * * *` (daily) | Drain pending `fulfillment_job` rows via the active connector |
| `/api/internal/worker/sla-check` | `0 */6 * * *` (every 6h) | Flag/escalate requests breaching SLA |
| `/api/internal/worker/approval-sla-check` | `0 * * * *` (hourly) | Approval-timeout escalation |
| `/api/internal/worker/intake-cleanup` | `*/30 * * * *` (every 30m) | Garbage-collect stale intake state |

Hobby plan caps cron frequency; upgrading the Vercel plan unlocks tighter schedules.
...
## Known issue — Sign-up "Failed to Fetch" (last touched, status unverified)

Sign-up at `https://aetherops-govw.vercel.app/sign-up` was returning "Failed to fetch". Before assuming the bug is current, hit the page and confirm — the fixes below may already have resolved it.

**Root causes found and patched:**
